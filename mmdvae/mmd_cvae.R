...
normal_loglik <- function(sample, mean, logvar, reduce_axis = 1L) {
  loglik <-  k_constant(0.5, dtype = tf$float64) * (
    tf$log(2 * k_constant(pi, dtype = tf$float64)) + 
    logvar + 
...
random_vector_for_generation <- k_random_normal(
  shape = list(num_examples_to_generate, latent_dim),
  dtype = tf$float64)

generate_random_digits <- function(
  epoch) {
  predictions <- decoder(random_vector_for_generation) %>% tf$nn$sigmoid()
  png(paste0("digits_epoch_", epoch, ".png"))
  par(mfcol = c(4, 4))
...
show_latent_space <- function(epoch) {
  ...
  ggsave(paste0("latentspace_epoch_", epoch, ".png"), width = 10, height = 10, units = "cm")
}
...
show_grid <- function(epoch) {
  ...
  for(i in 1:length(grid_x)){
    ...
    for(j in 1:length(grid_y)){
      ...
      column <- rbind(column, (decoder(z_sample) %>% tf$nn$sigmoid() %>% as.numeric()) %>% matrix(ncol = digit_size))
    }
...
for (epoch in seq_len(num_epochs)) {
  ...
  until_out_of_range({
    ...
    with(tf$GradientTape(persistent = TRUE) %as% tape, {
      ...
      logpz <- normal_loglik(z, k_constant(0, dtype = tf$float64), k_constant(0, dtype = tf$float64))
    ...
    optimizer$apply_gradients(purrr::transpose(list(encoder_gradients, encoder$variables)),
                              global_step = tf$train$get_or_create_global_step())
    optimizer$apply_gradients(purrr::transpose(list(decoder_gradients, decoder$variables)),
                              global_step = tf$train$get_or_create_global_step())
